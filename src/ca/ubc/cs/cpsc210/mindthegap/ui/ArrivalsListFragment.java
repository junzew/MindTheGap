...
public class ArrivalsListFragment extends ListFragment {
    ...
    public ArrayList<Arrival> getArrivalsForSelectedStationOnLineInDirection(String lineId, String travelDirn) {
        //TODO: Phase 2 Task 9
        // Use method from Task 8
        ArrayList<ArrivalBoard> arrivalBoards = ArrivalBoardListFragment.getArrivalBoardsForSelectedStation();
        ...
        for (ArrivalBoard ab: arrivalBoards) {
            if ((ab.getLine().getId().equals(lineId))&&(ab.getTravelDirn().equals(travelDirn))) {
                arrivals = getArrivals(ab);
            }
        }
        return arrivals;
    }
    // helper method for retrieving arrivals from an arrival board
    private ArrayList<Arrival> getArrivals(ArrivalBoard ab) {
        ArrayList<Arrival> arrivals = new ArrayList<>();
